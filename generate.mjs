...
for (const [key, value] of Object.entries(movement)) {
  ...
  movement2[key.replace("-", "U")] = value
    .replace("{#", "{#Shift_L(")
    .replace("}{^}", ")}{^}");

  movement2[key.replace("-", "EU")] = value
...
await fs.writeFile(
  "benno-steno-dicts/movement.json",
  JSON.stringify(movement3, null, "\t"),
  "utf-8",
);
...
await fs.writeFile(
  "benno-steno-dicts/casing.json",
  JSON.stringify(casing, null, "\t"),
  "utf-8",
);

await fs.writeFile(
  "benno-02-steno-dicts.json",
  JSON.stringify({ ...movement3, ...casing }, null, "\t"),
  "utf-8",
);
